 <%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="java.util.List, com.kh.forest.member.model.vo.Report, com.kh.forest.member.model.vo.PageInfo"%>
<%
   ...
   <article>
      <table class="resultTable">
         <tr>
            ...
            <th class="resultTableTh"><label>리뷰등록번호</label></th>
            <th class="resultTableTh"><label>신고회원명</label></th>
            <th class="resultTableTh"><label>신고회원번호</label></th>
            <th class="resultTableTh"><label>신고대상회원명</label></th>
            <th class="resultTableTh"><label>신고대상회원번호</label></th>
            <th class="resultTableTh"><label>신고일자</label></th>
            <th class="resultTableTh"><label>상태</label></th>
            <th class="resultTableTh"><label>상세</label></th>
         </tr>
         <% for(Report r : list){ %>
		<tr>
			<td class="resultTableTd"><input type="checkbox" class="reTbCheck" value="<%= r.getrBorderNo()%>"></td>
			<td class="resultTableTd"><%= r.getReportNo() %></td>
			<td class="resultTableTd"><%= r.getrBorderNo() %></td>
			<td class="resultTableTd"><%= r.getFromUserName() %></td>
			<td class="resultTableTd"><%= r.getFromUserNo() %></td>
			<td class="resultTableTd"><%= r.getToUserName() %></td>
			<td class="resultTableTd"><%= r.getToUserNo() %></td>
			<td class="resultTableTd"><%= r.getReportDate() %></td>
     ...
     <% if(pi != null){ %> 
      <div class="pagingArea" align="center">
      	<button onclick="location.href='<%= request.getContextPath()%>/reportSelectAll.me?currentPage=1'"><<</button>
      <% if(currentPage <= 1){ %>
      	<button disabled><</button>
      <%} else { %>
      	<button onclick="location.href='<%= request.getContextPath()%>/reportSelectAll.me?currentPage=<%= currentPage -1%>'"><</button>
      <% } %>
      ...
      <% for(int p =startPage ; p <= endPage; p ++ ) {  
    	...
    	<% } else { %>
    		<button onclick="location.href ='<%= request.getContextPath()%>/reportSelectAll.me?currentPage=<%= p%>'"><%= p %></button>
    	<% } %>  
      ...
      <button onclick="location.href='<%= request.getContextPath()%>/reportSelectAll.me?currentPage=<%= maxPage%>'">>></button>
      </div>
		<!-- --------------------- -->
		<hr>
		<label>회원관리</label>
		<div class="DDD">&nbsp;&nbsp;
			<button class="sendMail" id="waringMember">회원경고</button>
			<button class="sendMail" id="noWarning">신고반려</button>
		</div>
	</article>
 	...
 	$(function(){
 		$(".resultTable td").click(function(){
 			...
 			$(this).parent().children().eq(9).click(function(){
 				window.open('<%= request.getContextPath()%>/reportDetail.me?num='+num, "pop01", "top=100,left=300,width=500,height=500,location=no,scrollbars=no,status=no,resizable=0,titlebar=no,menubar=no,directories=no");
 	    ...
 	    $("#div6").click(function(){
 	    	location.href="<%= request.getContextPath()%>/selectAllWarningHistory.me";
 	    });
 	    $("#div5").click(function(){
 	    	location.reload();
 	    });
 		...
 		$("#noWarning").click(function(){
			...
			location.href='<%= request.getContextPath()%>/reportPass.me?num='+num;
 		});
