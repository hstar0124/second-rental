<%@ page language="java" contentType="text/html; charset=UTF-8"
	pageEncoding="UTF-8"
	import="java.util.List, com.kh.forest.member.model.vo.Member, com.kh.forest.member.model.vo.PageInfo"%>
<%
...
<body onload="ready('신고회원관리','회원관리','신고회원관리');">
		...
		<form>
			...
			<table class="searchGrid">
				...
				<tr>
					...
					</select> <input type="text" id="keyword" name="keyword" class="keyword">
					</td>
				</tr>
			</table>
			<br>
	...
	<article>
		<table class="resultTable">
			...
			<% for(Member m : list){ %>
			<tr>
				<td class="resultTableTd"><input type="checkbox" class="reTbCheck" value="<%= m.getUserNo()%>"></td>
				...
				<td class="resultTableTd"><button type="button" class="crmBtn"
						id="crmBtn">
						<img src="<%=request.getContextPath()%>/image/userimg/crm.PNG"
							width="20px">
					</button></td>
			</tr>
			<% } %>
		...
		<div class="pagingArea" align="center">
			<button
				onclick="location.href='<%= request.getContextPath()%>/warningMemberSelectAll.me?currentPage=1'"><<</button>
			<% if(currentPage <= 1){ %>
			<button disabled><</button>
			<%} else { %>
			<button
				onclick="location.href='<%= request.getContextPath()%>/warningMemberSelectAll.me?currentPage=<%= currentPage -1%>'"><</button>
			<% } %>
    	...
    	%>
			...
			<% } else { %>
			<button
				onclick="location.href ='<%= request.getContextPath()%>/warningMemberSelectAll.me?currentPage=<%= p%>'"><%= p %></button>
			<% } %>
			...
			<button
				onclick="location.href='<%= request.getContextPath()%>/warningMemberSelectAll.me?currentPage=<%= maxPage%>'">>></button>
		</div>
		<!-- --------------------- -->
		<hr>
		<label>회원관리</label>
		<div class="DDD">
			&nbsp;&nbsp;
			<button class="sendMail" id="waringMember">제제하기</button>
			<button class="sendMail" id="noWarning">제제복구</button>
		</div>
	</article>
	...
	<script>
 	$(function(){
 		...
 		$("#waringMember").click(function(){
			...
			location.href='<%= request.getContextPath()%>/memberBlackList.me?num='+num;
 		});
